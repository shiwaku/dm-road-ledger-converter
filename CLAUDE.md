...
加えて、記号・方向はスプライトにアイコンが無いコードを丸で描く。
豊中サンプルでは記号1,210件中221件（11コード）、方向1,473件中241件（8コード）が
こちらに回る（dm-sprite が166コードを持つ時点。**スプライトが増えるとこの数は変わるので、
書き換えるときは数え直すこと**）。数え方は `sprite.json` の `dm-` で始まるキー。
