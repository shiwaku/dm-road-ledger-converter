...
揃えていないのは2点。
1. 線・面の分類コード別の線種・線幅（公共測量標準図式の20パターン）を持たない（Issue #10）。
   道路台帳図の分類コードが自治体ごとに違うことに由来する
2. **記号の大きさをコードごとに補正しない。** 向こうは bbox の小さいアイコンを
   18.56px へ引き上げているが、こちらは倍率1のまま。ラスタを引き伸ばすと輪郭が
   ぼやけるうえ（`3401` 門は3.0倍で明確にぼけていた）、PDF図面より大きくなるため
   （図面0.60m に対し補正込みで約2.9m）。**図面に寄せる方針を優先している**（Issue #13）
...
豊中サンプルでは記号1,210件中221件、方向1,473件中588件がこちらに回る
（dm-sprite が158コードを持つ時点。**スプライトが増えるとこの数は変わるので、
...
4. **PDF図面との突き合わせ** — `data/.../PDF_57-08/DM-_57-08.pdf` はA0のベクタPDFで、
   テキストもパスも座標つきで取り出せる。図枠のグリッドラベル（`-48,800`〜`-48,400`／
   `-135,000`〜`-135,300`）から PDF pt → 平面直角座標のアフィン変換が作れる
   （実測 5.6685 pt/m ＝ 1:500）。ビューワ側は `map.getBounds()` の範囲を
