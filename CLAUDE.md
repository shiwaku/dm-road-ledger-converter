...
記号1,210件中221件（11コード）、方向1,473件中246件（9コード）がこちらに回る
（既定の「標準図式のみ」の場合。`VITE_DM_PROVIDERS=ext1,toyonaka` を付けると方向は
241件／8コードに減る）。dm-sprite が166コードを持つ時点の数で、**スプライトが増えると
変わるので、書き換えるときは数え直すこと**。数え方は `sprite.json` の `dm-` で始まるキー。
...
- **スプライトのキーは `dm-<コード>` だけではない。** 拡張DM（標準図式に無いコード）は
  ...
  **既定では標準図式のアイコンしか使わない。** 拡張DMの意匠は提供元ごとに違い、
  表示中のPMTilesがどの提供元のものかはビューワから判定できないため、
  `VITE_DM_PROVIDERS=ext1,toyonaka` のように利用者に明示してもらう
