...
const map = new maplibregl.Map({
  ...
  zoom: 17,
  minZoom: 4,
  // 表示上限は z22。タイルは z18 までしか焼いていないのでそれ以上はオーバーズームで
  // 描く（ベクタタイルなので情報は落ちない。細かくなるだけ）。
  // **上げるときは layers.ts の `TOP_ZOOM` も上げる。** 地上サイズ固定の補間の上端が
  // これを下回ると、そこから上は記号・文字・線幅が1段ごとに地上サイズ半分になる
  maxZoom: 22,
  maxPitch: 85,
